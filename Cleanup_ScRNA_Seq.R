...
setwd("/vast/palmer/home.grace/ab3478/scratch60")
...
################## get the scVelo spliced/unspliced estimate of spliced fraction
    ...
    spliced.mtx <- readr::read_delim(file.path(velocyto.path, "spliced.mtx"), col_names = FALSE,  delim = ' ', skip = 3)
    unspliced.mtx <- readr::read_delim(file.path(velocyto.path, "unspliced.mtx"), col_names = FALSE,  delim = ' ', skip = 3)
    ambiguous.mtx <- readr::read_delim(file.path(velocyto.path, "ambiguous.mtx"), col_names = FALSE,  delim = ' ', skip = 3)
...
ggplot(sub.ed@meta.data, aes(x=percSpliced.v, y=log10nUMI)) + 
    ...
    ylab("nUMI") +
    scale_fill_manual(values = c("F_HOX"="red", "F_NOX"="brown", "M_HOX"="yellow", "M_NOX"="green", "OF"="lightblue",
    "OM"="darkblue", "YF"="violet", "YM"="pink")) +
